...
do
    # your model path
    exp_name="./rl_output/checkpoint-1967"

    exp_name_clean=$(basename "$exp_name")
    echo "Processing category: $category with model: $exp_name_clean (STANDARD MODE)"
    
    train_file=$(ls ./data/Amazon18/Industrial_and_Scientific/minionerec_train_data/train/${category}*.csv 2>/dev/null | head -1)
    test_file=$(ls ./data/Amazon18/Industrial_and_Scientific/minionerec_train_data/test/${category}*11.csv 2>/dev/null | head -1)
    info_file=$(ls ./data/Amazon18/Industrial_and_Scientific/minionerec_train_data/info/${category}*.txt 2>/dev/null | head -1)
    ...
    metrics_file="$output_dir/metrics_${category}.txt"
    echo "Calculating metrics (NDCG / HR / AUC_pos_vs_neg)..."
    python ./calc.py \
        --path "$output_dir/final_result_${category}.json" \
        --item_path "$info_file" | tee "$metrics_file"
