...
def train(
    ...
    if resume_from_checkpoint and not os.path.exists(resume_from_checkpoint):
        raise FileNotFoundError(f"resume_from_checkpoint does not exist: {resume_from_checkpoint}")
    if resume_from_checkpoint and not model_path:
    ...
    # Match sft.sh-style behavior: run online when API key is provided.
    if os.environ.get("WANDB_API_KEY"):
        os.environ["WANDB_MODE"] = "online"
    else:
        os.environ["WANDB_MODE"] = "offline"
        print("WANDB_API_KEY is not set. Falling back to offline W&B mode.")
