...
'''
在这个项目里，每个码本向量维度是由 e_dim 定义的。
    定义位置：rq/rqvae.py 的参数 --e_dim
    默认值：32
    传递链路：rq/rqvae.py -> rq/models/rqvae.py -> rq/models/rq.py -> rq/models/vq.py
    最终生效处：rq/models/vq.py 里 nn.Embedding(self.n_e, self.e_dim)，第二个参数就是向量维度
    也就是说，按你当前 rq/rqvae.sh 的启动方式（没有覆盖 --e_dim），每个码本的向量维度就是 32。
另外默认 num_emb_list=[256,256,256]，表示有 3 个码本，每个码本 256 个向量、每个向量 32 维。
'''
'''
最佳碰撞率（Best Collision Rate）。
    定义：rq/trainer.py 里计算为
    collision_rate = (num_sample - unique_code_count) / num_sample
    含义：把每个样本的多级码本索引拼成一个离散码（如 12-34-56），统计有多少样本“撞码”（不同样本得到同一个码）
你的值：0.081455... 约等于 8.15%，即大约 8.15% 的样本发生了码碰撞
方向：这个指标通常越低越好（离散编码区分度更高）
'''
